...
// Class definition
var KTModalCreateProjectMainData = function () {
	// Variables
	var nextButton;
	var previousButton;
	var validator;
		...
		validator = FormValidation.formValidation(
			...
			fields: {
				...
				'sure_is_responsible': {
					validators: {
						notEmpty: {
							message: 'الرجاء التأكيد علي أن هذا الشخص هو المسؤل عن الجهه.'
						}
					}
				},
				'start_date': {
				...
				'mine_title': {
					validators: {
						callback: {
							...
							callback: function (input) {
								if ($('#inspection_visit').is(':visible')) {
									return input.value == '' ? false : true;
								}
							},
						},
	...
	var handleForm = function () {
		nextButton.addEventListener('click', function (e) {
			...
			var projectTypeId = document.querySelector('input[name="type_id"]:checked').value;
			if ((projectTypeId >= 2 && projectTypeId <= 5) || projectTypeId == 12) {
				document.getElementById('training_files').classList.add("d-none");
				document.getElementById('charity_list').classList.add("d-none");

				document.getElementById('research_survey').classList.remove("d-none");
				document.getElementById('google_map').classList.remove("d-none");
			} else if (projectTypeId == 9) {
				document.getElementById('charity_list').classList.remove("d-none");
				document.getElementById('training_files').classList.add("d-none");
				document.getElementById('google_map').classList.add("d-none");

				document.getElementById('research_survey').classList.add("d-none");
			} else if (projectTypeId == 14) {
				document.getElementById('training_files').classList.remove("d-none");
				document.getElementById('charity_list').classList.add("d-none");
				document.getElementById('research_survey').classList.add("d-none");
				document.getElementById('google_map').classList.add("d-none");
			} else {
	...
	return {
		...
		init: function () {
			...
			nextButton = KTModalCreateProject.getStepper().querySelector('[data-kt-element="main-next"]');
			previousButton = KTModalCreateProject.getStepper().querySelector('[data-kt-element="main-previous"]');
...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
	window.KTModalCreateProjectMainData = module.exports = KTModalCreateProjectMainData;
}
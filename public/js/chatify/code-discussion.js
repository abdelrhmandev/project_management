...
function routerPush(title, url,uId) {
  $("meta[name=url]").attr("content", url); 
  return window.history.pushState({}, title || document.title, ChatURL+'?User='+uId);
}
...
function sendMessage() {
  ...
  if (inputValue.length > 0 || hasFile) {
    ...
    formData.append("project_id",document.getElementById("Chat_project_id").value);
    $.ajax({
  ...
  if (messenger != 0 && !noMoreMessages && !messagesLoading) {
    ...
    $.ajax({
      ...
      data: {
        _token: access_token,
        id: id,
        project_id:projectId, // added by A.Magdy
        type: type,
        page: messagesPage,
      },
      dataType: "JSON",
...
$(document).ready(function () {
  ...
  $("body").on("click", ".messenger-list-item", function () {
    ...
    routerPush(document.title, `${url}/${userID}`, `${userID}`);
  ...
  $("body").on("click", ".favorite-list-item", function () {
    ...
    routerPush(document.title, `${url}/${uid}`, `${uid}`);
  });
...
// Project Manager  Account logged In , SHOW Client WINDOW CHAT PAGE --}}

 
  if(roleID == 2){
     DID = CUId; // user id = 28
    // Client  Account logged In , Project Manager WINDOW CHAT PAGE --}}
  }else if(roleID == 14){
     DID = '2'; // user id = 2;
  }
    $(".messenger-search").attr("disabled", "disabled"); 
    messengerSearch($(".messenger-search").val());
    $(".messenger-tab").hide();
    $('.messenger-tab[data-view="search"]').show();
    $(".messenger-listView").hide();            

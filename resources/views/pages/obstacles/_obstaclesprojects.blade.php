...
<div id="kt_content_container" class="container-xxl">
   ...
   <div class="row g-6 g-xl-9" id="projectList">
      ...
      <div class="col-md-6 col-xl-4 projectData" data-name="{{ $row->title }}">
         <!--begin:: Card header-->
         <a href="{{ route('obstacle.projectInfo',['ID' => $row->ID]) }}" class="card border-hover-primary">
            <div class="card-header border-0 pt-9">
               <div class="card-title m-0">
                  <div class="symbol symbol-50px w-50px bg-light"> <img src="{{ asset("storage/".$row->logo) }}" alt="{{ $row->title }}" class="p-3" /> </div>
               </div>
               <div class="card-toolbar"><span class="badge badge-light-primary fw-bold me-auto px-4 py-3">{{\App\Models\ProjectStatus::find($row->status_id)->trans}}</span></div>
            </div>
            <!--end:: Card header-->
            <!--begin:: Card body-->
            <div class="card-body p-9">
               ...
               <!--end::Name-->
               <!--begin::Description-->
               <p class="text-gray-400 fw-semibold fs-5 mt-1 mb-7">إجمالي العدد: {{ $row->cases_count ?? $row->building_count ?? '-'}}</p>
               <!--end::Description-->
               <!--begin::Info-->
      ...
      @empty @include('partials.alerts.empty',['title'=>$title]) @endforelse

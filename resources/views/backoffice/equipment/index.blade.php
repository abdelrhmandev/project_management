...
@section('content')
    ...
    <div id="kt_content_container" class="container-xxl">
        ...
        <div class="d-flex flex-wrap flex-stack mb-6">
            <!--begin::Heading-->
            <h3 class="fw-bold my-2">{{ __('project.search_case') }} <span
                    class="fs-6 text-gray-400 fw-semibold ms-1">{{ $status[0]->trans }}</span></h3>
            <!--end::Heading-->
            <!--begin::Actions-->
        ...
        <div class="row g-6 g-xl-9" id="projectList">
            @forelse($rows as $row)
                <div class="col-md-6 col-xl-4 projectData" data-name="{{ $row->title }}">
                    <!--begin:: Card header-->
                    <a href="#" onclick="return ShowConfirm('{{ $project_transaction_history->where("project_id", $row->id)->where("status_id", 8)->count() == 1? route($resource . '.edit', $row->id) : "#"}}','{{ $project_transaction_history->where("project_id", $row->id)->where("status_id", 17)->count() == 1? url('equipment/show-accounts', $row->id) : "#" }}','{{route('eq.elohade',['projectID'=>$row->id])}}')" class="card border-hover-primary">
                        <div class="card-header border-0 pt-9">
                            ...
                            <div class="card-toolbar">
                                <span class="badge badge-light-primary fw-bold me-auto px-4 py-3">{{ $status[$row->status_id - 1]->trans }}</span>
                            </div>
                        </div>
                        <!--end:: Card header-->
                        ...
                        <div class="card-body p-9">
                            ...
                            <div class="h-4px w-100 bg-light mb-5" data-bs-toggle="tooltip"
                                ...
                                <div class="bg-primary rounded h-4px" role="progressbar"
                                    style="width:{{ $row->progress_bar }}%" aria-valuenow="100" aria-valuemin="0"
...
@section('scripts')
    <script>
        function ShowConfirm(editUrl, showaccountsURL,elohadUrl) {
            Swal.fire({
                title: 'هل تريد العمل على توفير التجهيزات للمشروع أو إضافة الحسابات في برنامج كاشف؟',
                icon: 'info',
                html: '<a class="btn btn-light-warning font-weight-bold" href="' + showaccountsURL +
                    '"><i class="bi bi-person-plus"></i>الحسابات</a><a class="btn btn-light-primary font-weight-bold" href="' + editUrl +
                    '"><i class="bi bi-bag-plus"></i>التجهيزات</a><a class="btn btn-light-primary font-weight-bold" href="' + elohadUrl +
                    '"><i class="bi bi-bag-plus"></i>العهد</a></p>',

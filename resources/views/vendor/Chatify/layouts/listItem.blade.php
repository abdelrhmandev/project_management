...
@if($get == 'saved')
    <table class="messenger-list-item m-li-divider" data-contact="{{ Auth::user()->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ Auth::user()->id }}" data-type="user">{{ __('site.saved_messages') }} <span>{{ __('site.you') }}</span></p>
                <span>{{ __('site.save_messages_secretly') }}</span>
            </td>
        </tr>
    </table>
@endif
    ...
    <tr data-action="0">
        ...
        <span>
            ...
            {!!
                ...
                ? '<span class="lastMessageIndicator">'.__('site.you').' :</span>'
                : ''
            !!}
            {{-- Last message body --}}
            ...
            !!}
            @else
            <span class="fas fa-file"></span> {{ __('site.attachment') }}
            @endif
        </span>
...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <td>
            <p data-id="{{ $user->id }}" data-type="user">
            {{-- {{ strlen($user->name) > 12 ? trim(substr($user->name,0,12)).'..' : $user->name }}  ORIGINAL CODE--}}
            {{ $user->name }}  
        </td>
